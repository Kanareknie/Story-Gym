...
class RandomizerResult(models.Model):
    user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name='randomizer_results'
    )
    ...
    def __str__(self):
        return (
            f"Randomizer result for {self.user.username} "
            f"at {self.created_on}"
        )
...
class Story(models.Model):
    ...
    randomizer = models.OneToOneField(
        RandomizerResult,
        on_delete=models.CASCADE,
        related_name='story'
    )
...
class Comment(models.Model):
    ...
    # Adding the author name display method
    # to handle both registered and deleted users
